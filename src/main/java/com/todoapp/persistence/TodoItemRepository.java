...
public interface TodoItemRepository extends CrudRepository<TodoItem, UUID> {
     ...
     */
    @Query("SELECT * FROM TODO_ITEM ORDER BY IMPORTANT DESC, POSITION ASC")
    @Override
    @NonNull
     ...
     */
    @Query("SELECT * FROM TODO_ITEM WHERE LIST_ID = :listId AND COMPLETED = :completed " +
            "ORDER BY IMPORTANT DESC, POSITION ASC, CREATED_AT DESC")
     ...
     */
    @Query("SELECT * FROM TODO_ITEM WHERE LIST_ID = :listId ORDER BY IMPORTANT DESC, POSITION ASC")
