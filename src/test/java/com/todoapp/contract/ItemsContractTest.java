...
class ItemsContractTest {
    ...
    private String toggleImportant(String itemId) throws Exception {
        String response = mockMvc
                .perform(
                        post("/api/items/{itemId}/important", itemId))
                .andExpect(status().isOk())
                .andReturn()
                .getResponse()
                .getContentAsString();

        return objectMapper.readTree(response).get("important").asText();
    }
    ...
    @Test
    void getItems_shouldReturnOrderedByImportant() throws Exception {
        String listId = createTestList();
        createTestItem(listId);
        String secondItemID = createTestItem(listId);
        createTestItem(listId);

        //toggle important for the second item, so we can test it to be the first one
        String important = toggleImportant(secondItemID);
        ...
        mockMvc
                ...
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$").isArray())
                .andExpect(jsonPath("$[0].important").value(Boolean.valueOf(important)))
